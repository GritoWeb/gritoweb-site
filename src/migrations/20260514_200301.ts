...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`pages_blocks_blog_listing\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`pages_blocks_blog_listing_order_idx\` ON \`pages_blocks_blog_listing\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_blog_listing_parent_id_idx\` ON \`pages_blocks_blog_listing\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_blog_listing_path_idx\` ON \`pages_blocks_blog_listing\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_blog_listing_featured_post_idx\` ON \`pages_blocks_blog_listing\` (\`featured_post_id\`);`)
  await db.run(sql`CREATE TABLE \`_pages_v_blocks_blog_listing\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_blog_listing_order_idx\` ON \`_pages_v_blocks_blog_listing\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_blog_listing_parent_id_idx\` ON \`_pages_v_blocks_blog_listing\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_blog_listing_path_idx\` ON \`_pages_v_blocks_blog_listing\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_blog_listing_featured_post_idx\` ON \`_pages_v_blocks_blog_listing\` (\`featured_post_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`pages_blocks_blog_listing\`;`)
  await db.run(sql`DROP TABLE \`_pages_v_blocks_blog_listing\`;`)
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`portfolio_tags_id\`) REFERENCES \`portfolio_tags\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "portfolios_id", "portfolio_tags_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "portfolios_id", "portfolio_tags_id" FROM \`payload_locked_documents_rels\`;`)
